...
import Footer from './Footer'
 class AdminList extends Component {
  ...
  componentDidMount() {
    axios
      .get("http://127.0.0.1:8000/getAlladmins")
      .then((response) => {
...
deleteadmin(id,e)
{
  ...
  axios
      .delete("http://127.0.0.1:8000/deleteadmin", {data})
      .then((response) => {
        // console.log(response);
...
<h3 style={{color:'red',marginLeft:'400px',marginTop:'5px'}}>{this.state.searcherrormessage}</h3>
  ...
  <ul className={Styles.data}>
    ...
    <li>
		<div style={{marginLeft:'200px'}}><span  style={{color:'black'}}><a  onClick={(e)=>this.deleteadmin(post.uid,e)}>Delete</a></span></div>
    </li>
 </ul>
</div>
